...
JavaScript中，正则表达式的字面量具备修饰符` i` 和 `g` 和 `m` 和 `u`    
...
事实上，replace可以接受函数，函数的形参指代被匹配的字符串：

```js
replace(/\\\/|\\\\|\\n|\\r|\\t|\\f|\\0/g,function(entity){
    switch(entity){
        case "\\\/":
            return '\/'
            break;
...
`asd123
Asd123
aSd321`.match(/^(?!Asd).*/mg)//匹配不以Asd开头的行["asd123", "aSd321"]
```
