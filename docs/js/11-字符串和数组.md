...
* 返回单个字符的`unicode`编码 和通过编码返回单个字符
...
* 拼接数组，类似于字符串`concat`，返回拼接的数组，原数组不变，多个参数拼接添加
...
* `every`对每一个项判断，都满足返回`true`，接受一个回调函数(以参数形式传入的函数，此函数依次接收数组每一项作为参数，返回`true/false`)
