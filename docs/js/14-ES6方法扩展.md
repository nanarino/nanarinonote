...
* `parseInt` 取整 es6中被置于`Number`对象上

```js
parseInt==Number.parseInt;//true
```

* `parseFloat` 取浮点数 可以去掉单位 es6中被置于`Number`对象上

```js
Number.parseFloat==parseFloat;//true
```

* `Number.isNaN` 严格判断是不是`NaN`
...
- `every` 对每一个项判断，都满足返回`true`，接受一个回调函数(以参数形式传入得函数，此函数依次接收数组每一项作为参数，返回true/false)
...
* `map` 将数组中的每一项进行操作，然后返回，**改变原数组**，格式和`filter`一致
...
* 对象中的`get`与`set`方法，获取元素的值和设置值得不同操作。
...
* 属性的特点：
  ...
  * 属性特点可修改性（configurable）
  ...
  2. `Object.defineProperty`添加。

     参数： 对象 属性名 属性值性质对象列表。可修改性，

     ​	用`defineproperty`设置时默认为`false`，
  ...
  `get`和`set`在对属性操作(获取，修改)得时候会触发得操作，
