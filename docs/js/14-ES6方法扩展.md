...
数字的原型方法：`toFixed(f)` 保留小数点后位数

Math对象方法：`round(f)` `ceil(f)` `floor(f)` `rinc(f)`

Number对象方法：`parseInt(f)`
