...
在不借助form表单时，一般需要设置请求头，作用与form.enctype一样

```js
xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded")
```

除了GET请求，其他动作一般还会有请求体。
...
其他方法请求，body可以传字符串或formdata对象。传字符串时最好指定`content-type`。
...
`res.ok`可以判断请求是否正常。无论正常与否，都可以使用`res.json()`解析结果。此外还有`res.state`等
...
get请求时只有2个参数，不接受data参数。params是地址栏的query参数，不限于get请求。
