...
class Color_info(Info):
    '''带颜色的信息类'''
    def __init__(self, msg, color):
        super().__init__(msg)
        self.color = color
    def out(self):
        print(f'{getattr(Fore, self.color)}{self.msg}{Fore.RESET}')

red_info = Color_info('我爱你','RED')
red_info.out()
```

重写了父类的out方法，这里的getattr方法将在后文介绍。    
调用`Color_info('我爱你','RED').out()`即可在控制台打印出红色的<span style='color:red'>我爱你</span>。

::: tip 派生基本类
无法在`__init__`时对基本类（如str）派生属性    
且派生时`super().__init__`不需要传入参数。
:::

派生基本类时，退而求其次的使用方法来添加属性：
...
class Color_str(str):
    ...
    def __init__(self, msg):
        """事实上，这个__init__不需要写，msg就是字符串的本体了"""
        self.msg = msg
        super().__init__()
	...
	def out(self):
        print(f'{getattr(Fore, self.color)}{self}{Fore.RESET}')
        """f字符串的{self}其实是调用了self.__str__()没实现，向父类查找"""

red_str = Color_str('我爱你').set_color('RED')
print(red_str)
red_str.out()
```

`print()`会调用str类的`__str__()`魔术方法打印出msg本体，除非重写它

而`red_str.out()`正确打出了预期的带颜色的msg
