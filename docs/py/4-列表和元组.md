...
`sorted(iterable)`临时排序，不限于列表 ，可选参数key进行操作
...
`.sort(cmp=None, key=None, reverse=False)`永久排序

`.reverse()`倒序，相当于`li[:] = li[::-1]`的效果

::: tip 列表的sort方法和全局函数sorted

它们本质是**快排算法**两两比较大小

重新定义的比较大小的函数可以用cmp参数传入

它们的参数几乎一致。不同的是：

列表的sort方法无返回值，直接修改列表对象

全局函数sorted，返回临时排序后的列表
:::
