...
class my_list(list):
    def __init__(self,*args,**kwargs):
        '''这里没有给self派生属性，__init__方法可以不写，会继承
        在调用super().__init__(*args,**kwargs)之前，列表会是空的[]'''
        print(self)
...
class my_str(str):
    def __init__(self,*args,**kwargs):
        '''str类即使派生属性，也不需要调用super().__init__()
        因为早在str.__new__方法里str就已经构造完毕，'''
        print(self)
    def __str__(self):
        '''这是重写的父类的魔术方法，它会被print调用'''
        return f'<my_str {self!r}>'

s = my_str(b'\xe4\xbd\xa0\xe5\xa5\xbd','utf-8') # <my_str '你好'>
print(s) # <my_str '你好'>
```
